...
import com.example.slapimage.xededitor.xededitor.MainActivity.file.FileManager.Companion.findGitRoot
...
suspend fun updateMenu(tabFragment: TabFragment?) = withContext(Dispatchers.Main) {
    ...
    val menu = XEDMainActivity.activityRef.get()?.menu
    if (menu != null) {
...
private suspend fun updateEditor(
    ...
    menu: Menu,
    showItems: Boolean = fragment != null && XEDMainActivity.activityRef.get()?.adapter?.tabFragments?.isNotEmpty() == true
) {

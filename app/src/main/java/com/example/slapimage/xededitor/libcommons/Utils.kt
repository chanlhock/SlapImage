...
fun dialog(context: Activity? = XEDMainActivity.activityRef.get(), title: String?, msg: String?, onCancel:((DialogInterface)-> Unit)? = null, onOk:((DialogInterface)-> Unit)? = null){
    if (context == null){
...
fun composeDialog(context: Activity? = XEDMainActivity.activityRef.get(),content:@Composable (AlertDialog?) -> Unit){
    if (context == null){
        throw IllegalArgumentException("context cannot be null")
        return
    }

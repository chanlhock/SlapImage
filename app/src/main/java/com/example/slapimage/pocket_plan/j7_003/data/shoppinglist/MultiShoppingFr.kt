...
import com.example.slapimage.databinding.DialogAddItemBinding
import com.example.slapimage.databinding.DialogAddShoppingListBinding
import com.example.slapimage.databinding.FragmentMultiShoppingBinding
...
class MultiShoppingFr : Fragment() {
    ...
    private lateinit var myActivity: PocketPlanMainActivity
    ...
    override fun onCreateView(
        ...
        myActivity = activity as PocketPlanMainActivity
    ...
    private fun updateTabs() {
        if (PocketPlanMainActivity.shoppingListWrapper.size == 1) {
            tabLayout.visibility = View.GONE
        } else {
            tabLayout.visibility = View.VISIBLE
        }
        tabLayout.removeAllTabs()
        PocketPlanMainActivity.shoppingListWrapper.forEach {
    ...
    private fun dialogRenameCurrentList() {
        ...
        //show current name
        val oldName = PocketPlanMainActivity.shoppingListWrapper[currentpos].first
        ...
        dialogAddShoppingListBinding.btnAddShoppingList.setOnClickListener {
            ...
            val taken = PocketPlanMainActivity.shoppingListWrapper.contains(newName)
            ...
            PocketPlanMainActivity.shoppingListWrapper.rename(oldName, newName)
    ...
    private fun dialogAddShoppingList() {
        ...
        dialogAddShoppingListBinding.btnAddShoppingList.setOnClickListener {
            val newName = dialogAddShoppingListBinding.etAddShoppingList.text.toString()
            val addResult = PocketPlanMainActivity.shoppingListWrapper.add(newName)
            ...
            val newFr = createShoppingFrInstance(
                newName,
                PocketPlanMainActivity.shoppingListWrapper.getListByName(newName)!!
            )
            shoppingFragments.add(newFr)
            ...
            shoppingPager.currentItem = PocketPlanMainActivity.shoppingListWrapper.size - 1
            myAlertDialog?.dismiss()
        }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        when (item.itemId) {
            R.id.item_shopping_delete_list -> {
                ...
                val action: () -> Unit = {
                    PocketPlanMainActivity.shoppingListWrapper.remove(activeShoppingFr.shoppingListName)
    ...
    fun refreshItemNamesAndAutoCompleteAdapter() {
        ...
        PocketPlanMainActivity.itemNameList = ArrayList()

        //add userItemNames to itemNameList
        myActivity.userItemTemplateList.forEach {
            PocketPlanMainActivity.itemNameList.add(it.n)
        }

        //add all regular items to itemNameList
        myActivity.itemTemplateList.forEach {
            if (!PocketPlanMainActivity.itemNameList.contains(it.n)) {
                PocketPlanMainActivity.itemNameList.add(it.n)
            }
        }

        //initialize custom arrayAdapter for autocompletion
        val itemNameClone = PocketPlanMainActivity.itemNameList.toMutableList()
    ...
    fun preloadAddItemDialog(passedActivity: PocketPlanMainActivity, layoutInflater: LayoutInflater) {
        ...
        dialogAddItemBinding.btnAddItemToList.setOnClickListener {
            ...
            if (PocketPlanMainActivity.previousFragmentStack.peek() == FT.SHOPPING) {
                ...
                PocketPlanMainActivity.shoppingListWrapper[0].second.add(item)
                Toast.makeText(
                    myActivity,
            ...
            if (PocketPlanMainActivity.previousFragmentStack.peek() == FT.POCKETPLAN_HOME || SettingsManager.getSetting(
    ...
    private fun updateDeleteListIcon() {
        myMenu.findItem(R.id.item_shopping_delete_list)?.isVisible =
            PocketPlanMainActivity.shoppingListWrapper.size > 1
    }
    ...
    private inner class ScreenSlidePagerAdapter(fa: FragmentActivity) : FragmentStateAdapter(fa) {
        override fun getItemCount(): Int = PocketPlanMainActivity.shoppingListWrapper.size

...
class TableauPile(var cards: MutableList<Card> = mutableListOf()) {
    ...
    fun removeCards(tappedIndex: Int) {
        if (tappedIndex >= 0 && tappedIndex < cards.size) { // Add safety check
            val cardsToRemove = cards.subList(tappedIndex, cards.size)
            cards.removeAll(cardsToRemove.toSet()) // Safer removal
            if (cards.isNotEmpty()) {

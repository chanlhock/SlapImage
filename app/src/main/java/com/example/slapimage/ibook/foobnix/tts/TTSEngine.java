...
import com.example.slapimage.ibook.foobnix.mobi.parser.MobiParserIS;
...
public class TTSEngine {
    ...
    OnInitListener listener = new OnInitListener() {
        ...
        public void onInit(int status) {
            ...
            if (status == TextToSpeech.ERROR) {
                Toast.makeText(ApplicationClass.context, R.string.msg_unexpected_error, Toast.LENGTH_LONG).show();
            }
    ...
    public TextToSpeech getTTS(OnInitListener onLisnter) {
        ...
        synchronized (helpObject) {
            ...
            ttsEngine = new TextToSpeech(ApplicationClass.context, onLisnter);
        }
    ...
    public void stop(MediaSessionCompat mediaSessionCompat) {
        ...
        if (!AppState.get().allowOtherMusic) {
            try {
                ...
                AudioManager mAudioManager = (AudioManager) ApplicationClass.context.getSystemService(Context.AUDIO_SERVICE);
    ...
    public TextToSpeech setTTSWithEngine(String engine) {
        shutdown();
        synchronized (helpObject) {
            ttsEngine = new TextToSpeech(ApplicationClass.context, listener, engine);
        }
        return ttsEngine;
    }

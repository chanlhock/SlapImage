            ...
            return new LoadData<>(sourceKey, new DataFetcher<Bitmap>() {
                ...
                public void loadData(@NonNull Priority priority, @NonNull DataCallback<? super Bitmap> callback) {
                    try {
                        ...
                        final InputStream stream = ImageExtractor.getInstance(ApplicationClass.context).getStream(s, null);
                        if (stream instanceof InputStreamBitmap) {
                            Bitmap bitmap = ((InputStreamBitmap) stream).getBitmap();
                            if (bitmap == null) {
                                Thread.sleep(250);
                                bitmap = ImageExtractor.getInstance(ApplicationClass.context).proccessOtherPage(s);
                                if (bitmap == null) {

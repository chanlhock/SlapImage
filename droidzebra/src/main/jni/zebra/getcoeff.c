...
#include "search.h"
#include "texts.h"
#include <stdint.h> // For uint64_t for fixed-width integer type
#include <time.h>
...
/*
   rdtsc - Replaced with a portable high-resolution timer for ARM/Android.
           Note: This will not be an actual cycle count like rdtsc, but
           a high-resolution time in nanoseconds since an arbitrary point.
*/
static uint64_t // Changed to uint64_t for portability and accuracy
rdtsc( void ) {
#if defined(ANDROID) || defined(__linux__) || defined(__APPLE__) // Cover common non-Windows/non-ancient x86 systems
    struct timespec ts;
    // CLOCK_MONOTONIC provides a non-decreasing timer, suitable for measuring intervals.
    if (clock_gettime(CLOCK_MONOTONIC, &ts) == 0) {
        // Convert seconds and nanoseconds to total nanoseconds
        return (uint64_t)ts.tv_sec * 1000000000ULL + ts.tv_nsec;
    } else {
        // Handle error, e.g., print a message or return 0
        // perror("clock_gettime failed");
        return 0; // Or some other appropriate error value
    }
#elif defined(_WIN32) // For Windows (if supported in your cross-platform build)
    // On Windows, QueryPerformanceCounter is the equivalent
    // You'd need to include <windows.h> and link with advapi32.lib
    ...
    return 0;
#else // Fallback for other or unknown systems, or older x86 where rdtsc is explicitly desired
    // This part is retained only if you *definitely* need rdtsc on specific x86 builds
    // For general portability on Android/ARM, this block is usually not reached.
    unsigned int lo, hi;
    asm volatile("rdtsc" : "=a" (lo), "=d" (hi));
    return ((uint66_t)hi << 32) | lo;
#endif
}
